...
function normalscore:eq1i(sigma, name)
	...
	local t_L, t_R = range[1] - self.mean, range[2] - self.mean
...
function normalscore:update()
	...
	if self.miss_count > 0 and self.hit_count > 0 then
		local tau_t_0 = erfunc.erfinv(self.hit_count / N)

		local sum = 0
		local sum_weights = 0
		for name, range in pairs(self.ranges) do
			local t_L, t_R = range[1], range[2]
			sum = sum + (t_R - t_L) / (2 * tau_t_0 * math.sqrt(2)) * self.hit_counts[name]
	...
	local score_squared = self.hit_count / math.max(N, 1) * (self.variance - mean ^ 2) + self.miss_addition
