...
local normalscore = require("normalscore2_2")
...
local values1 = norm_values(1e6, 0, sigma)
local range1 = {-0.02, 0.02}
for _, v in ipairs(values1) do
	press(v, range1, "range1")
end

local values2 = norm_values(1e6, 0, sigma)
local range2 = {-0.03, 0.03}
...
assert(math.abs(ns.score_adjusted - sigma) / sigma < 0.01)
